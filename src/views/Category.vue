...
const categoryName = computed(() => t(`categories.${route.params.category}`) || route.params.category)
const filteredBooks = computed(() => allBooks.value.filter(b => b.category === route.params.category))
...
onMounted(loadBooks)
watch(() => route.params.category, loadBooks)
</script>
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8 flex items-center justify-between flex-wrap gap-4">
      <div>
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ categoryName }}</h2>
        <p v-if="!loading" class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ filteredBooks.length }} {{ t('book.count') }}</p>
      </div>
      <select
